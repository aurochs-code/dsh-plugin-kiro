...
function isToolUpdate(params) {
    const update = isRecord(params.update) ? params.update : params;
    const kind = update.sessionUpdate ?? update.type ?? update.kind;
    return kind === 'tool_call' || kind === 'ToolCall' || kind === 'tool_call_update' || kind === 'ToolCallUpdate';
}
/**
...
export class KiroAcpClient {
    ...
    async initialize(signal) {
        await this.request('initialize', {
            protocolVersion: 1,
            clientCapabilities: {},
            clientInfo: { name: 'dsh-plugin-kiro', version: '0.2.0' },
        }, signal);
    }
    ...
    receive(line) {
        ...
        if (message.method !== 'session/update' && message.method !== 'session/notification') {
            if (message.id !== undefined)
                this.replyUnsupported(message.id);
            return;
        }
        ...
        if (text !== undefined)
            queue.push(text);
        if (isToolUpdate(params))
            queue.fail(new KiroCliError('Kiro requested a tool call; dsh-plugin-kiro only supports text generation'));
    }
    replyUnsupported(id) {
        try {
