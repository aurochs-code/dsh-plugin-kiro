...
import type {
  KiroAcpClientHandlers,
  KiroAcpCreateTerminalRequest,
  KiroAcpTerminal,
} from './acp.js'
...
function stringField(value: Record<string, unknown>, name: string): string {
  ...
  if (typeof field !== 'string') throw new KiroCliError(`DSH ${name} result was malformed`)
  return field
}

function booleanField(value: Record<string, unknown>, name: string): boolean {
  const field = value[name]
  if (typeof field !== 'boolean') throw new KiroCliError(`DSH ${name} result was malformed`)
  return field
}
...
function nullableNumberField(value: Record<string, unknown>, name: string): number | null {
  ...
  if (typeof field !== 'number') throw new KiroCliError(`DSH ${name} result was malformed`)
  return field
}
...
function terminalCommand(request: KiroAcpCreateTerminalRequest): string {
  return [request.command, ...(request.args ?? [])].map(quoted).join(' ')
}

function terminalValue(value: unknown): KiroAcpTerminal {
...
function readValue(value: unknown): string {
  if (!isRecord(value) || !Array.isArray(value.lines)) throw new KiroCliError('DSH read result was malformed')
  return value.lines.map((line) => {
    if (!isRecord(line)) throw new KiroCliError('DSH read result was malformed')
 ...
 */
export function createDshToolHandlers(
  ctx: Context,
  sessionId: GenerateOptions['sessionId'],
): KiroAcpClientHandlers | undefined {
  if (sessionId === undefined || ctx.agents.get(sessionId) === undefined) return undefined

  let callIndex = 0
  const run = async (name: string, args: Record<string, unknown>, signal?: AbortSignal): Promise<unknown> => {
    const agent = ctx.agents.get(sessionId)
    if (agent === undefined) throw new KiroCliError('the DSH session is no longer available for this Kiro ACP request')
    ...
    if (result.isError) throw new KiroCliError(`DSH ${name} rejected the Kiro ACP request: ${result.error.message}`)
    return result.value
  }
