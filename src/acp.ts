...
function isToolUpdate(params: Record<string, unknown>): boolean {
  const update = isRecord(params.update) ? params.update : params
  const kind = update.sessionUpdate ?? update.type ?? update.kind
...
export class KiroAcpClient {
  ...
  private receive(line: string): void {
    ...
    if (isToolUpdate(params)) queue.fail(new KiroCliError('Kiro requested a tool call; dsh-plugin-kiro only supports text generation'))
  }
