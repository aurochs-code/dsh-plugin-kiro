...
export function toKiroConversation(options: Pick<GenerateOptions, 'messages' | 'system'>): KiroConversation {
  const system = options.system?.trim()
  const messages: KiroConversationMessage[] = []
...
function sameMessage(left: KiroConversationMessage, right: KiroConversationMessage): boolean {
 ...
 */
export function toKiroContinuationPrompt(
  conversation: KiroConversation,
  previous: KiroConversation,
  previousResponse: string,
): string | undefined {
  if (conversation.system !== previous.system || conversation.messages.length <= previous.messages.length) return undefined
  if (!previous.messages.every((message, index) => sameMessage(message, conversation.messages[index]!))) return undefined
  ...
  if (first?.role === 'assistant') {
    ...
    if (first.content !== previousResponse) return undefined
